...
# Tabs trên cùng
tabs = st.tabs(["🏠 Trang Chủ", "📊 Phương Pháp Phân Cụm", "📈 Kết Quả Dự Án", "🧪 Trải Nghiệm sản phẩm"])
...
with tabs[3]:
    ...
    if mode == "🔢 Nhập thủ công":
        ...
        if sub_mode == "Nhập Recency/Frequency/Monetary":
            ...
            customer_id_input = st.text_input("Nhập Customer ID, ví dụ như **2433**, **2193**:")
    ...
    if st.button("Phân cụm dựa vào KMeans & RFM") and input_data is not None:
        try:
            ...
            st.exception(e)
    st.markdown("---")
    st.subheader("📥 Hoặc tải lên danh sách nhiều khách hàng để phân cụm")
